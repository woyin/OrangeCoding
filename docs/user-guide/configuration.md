...
> OrangeCoding 使用 JSONC（带注释的 JSON）格式进行配置，支持项目级和全局级的层级覆盖。
...
OrangeCoding 使用 JSONC（JSON with Comments）格式，在标准 JSON 基础上支持：
...
| 级别 | 路径 | 说明 |
|------|------|------|
| **全局配置** | `~/.orangecoding/config.jsonc` | 用户级全局设置 |
| **项目配置** | `.opencode/orangecoding.jsonc` | 项目级设置 |
...
```
内置默认值 < 全局配置 (~/.orangecoding/config.jsonc) < 项目配置 (.opencode/orangecoding.jsonc)
```

### 适合放在全局配置中的设置

```jsonc
// ~/.orangecoding/config.jsonc
{
  // API Key 通常是全局的
  "providers": {
    "openai": {
...
| 提供者 | 标识 | 可用模型 | 特殊功能 |
|--------|------|---------|---------|
| **OpenAI / GPT** | `openai`, `gpt` | gpt-5.1 | 函数调用、流式输出、`reasoning_effort` |
| **Anthropic / Claude Opus** | `anthropic`, `claude`, `opus` | claude-opus-4-7 | Anthropic Messages API、thinking budget |
| **Kimi / Moonshot** | `kimi`, `moonshot` | kimi-k2.6 | OpenAI-compatible Chat Completions |
| **GLM / Z.AI** | `glm`, `bigmodel`, `zhipu` | glm-5.1 | OpenAI-compatible Chat Completions、thinking mode |
...
- Anthropic Claude 模型列表：<https://platform.claude.com/docs/en/about-claude/models/overview>
- Kimi OpenAI-compatible API：<https://platform.kimi.ai/docs/api/overview>
...
```jsonc
{
  ...
  "providers": {
    ...
    "kimi": {
      ...
      // base_url 可省略；内置默认值为 https://api.moonshot.ai/v1
    },
    "glm": {
      "api_key": "${ZAI_API_KEY}",
      // base_url 可省略；内置默认值为 https://api.z.ai/api/paas/v4
    },
    "deepseek": {
...
`gpt` 会复用 `providers.openai` 的 API key；`opus` 会复用 `providers.anthropic` 的 API key。也可以直接把 `default_provider` 写成 `openai` 或 `anthropic`。
...
OrangeCoding 使用模型角色系统为不同类型的任务选择合适的模型：
...
{
  "model_fallback": {
    "claude-opus-4-7": ["gpt-5.1", "glm-5.1"],
    "gpt-5.1": ["claude-opus-4-7", "kimi-k2.6"],
    "deepseek-coder": ["deepseek-chat", "gpt-5.1"]
  }
}
```
...
`harness.reasoning_effort` 会映射到 OpenAI GPT 的 `reasoning_effort`；对 Kimi/GLM 这类使用 `thinking` 扩展的 OpenAI-compatible provider，会发送 `thinking: {"type":"enabled"}`。`harness.reasoning_budget_tokens` 会映射到 Anthropic provider 的 `thinking.budget_tokens`。
...
| 配置 | OpenAI-compatible | Anthropic-compatible |
|------|-------------------|----------------------|
| `reasoning_effort` | GPT: `none`, `low`, `medium`, `high`; Kimi/GLM: 非 `none` 时启用 `thinking` | 不直接发送 |
| `reasoning_budget_tokens` | 不直接发送 | `thinking.budget_tokens` |
...
OrangeCoding 的 Intent Gate 系统自动分类用户意图：
...
```jsonc
{
  "hooks": {
    ...
    "post_message": [
      {
        "priority": 20,
        "script": ".orangecoding/hooks/log-message.sh",
        "timeout": 5                     // 脚本超时（秒）
      }
    ],
    ...
    "pre_tool_call": [
      {
        "priority": 1,                  // 最高优先级
        "script": ".orangecoding/hooks/security-check.sh",
...
`.orangecoding/hooks/security-check.sh`：
...
|------|--------|------|
| 内置 | 最低 | 程序内部 |
| 用户全局 | 中 | `~/.orangecoding/skills/` |
| 项目 | 最高 | `.orangecoding/skills/` |
...
```jsonc
{
  "logging": {
    ...
    "file": "~/.orangecoding/logs/OrangeCoding.log",   // 日志文件路径
...
```jsonc
// .opencode/orangecoding.jsonc — 完整配置示例
{
  // === AI 提供者配置 ===
  "ai": {
    "provider": "anthropic",
