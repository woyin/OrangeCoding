...
> OrangeCoding 内置 11 个专业 Agent，每个 Agent 拥有特定的角色、模型、工具权限和使用场景。
...
**角色定位：**
Sisyphus 是 OrangeCoding 的核心编排 Agent，以希腊神话中永不放弃的西西弗斯命名。它拥有全部工具权限，负责理解用户意图、分解任务并协调其他 Agent 完成工作。
...
在 `.opencode/OrangeCoding.jsonc` 中覆盖 Agent 默认配置：
