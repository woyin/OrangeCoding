...
> CEAIR 内置多种工作流模式，支持从手动交互到全自动开发的灵活切换。
...
UltraWork 是 CEAIR 的全自动开发模式，能够自主完成从需求分析到代码验证的完整开发流程。
...
Prometheus 是 CEAIR 的战略规划器，通过结构化的状态机流程生成高质量的开发计划。
...
Boulder 是 CEAIR 的会话连续性系统，以西西弗斯推动的巨石命名。它记录工作状态，支持跨会话的任务延续。
...
CEAIR 通过 `ceair-mesh` 模块支持多 Agent 间的任务协商和动态重分配。
