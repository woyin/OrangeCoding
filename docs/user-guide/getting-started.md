...
> OrangeCoding — 基于 Rust 构建的终端 AI 编程代理
...
OrangeCoding 要求 **Rust 1.75** 或更高版本。推荐使用 `rustup` 管理 Rust 工具链：
...
OrangeCoding 使用 JSONC（带注释的 JSON）格式进行配置。首次运行时，建议创建项目级配置文件。

### 初始化项目配置

在你的项目根目录下创建 `.opencode/orangecoding.jsonc`：

```bash
mkdir -p .opencode
```

创建最小配置文件 `.opencode/orangecoding.jsonc`：
...
OrangeCoding 按以下顺序查找和合并配置（后者覆盖前者）：

1. **内置默认值** — 程序内部的默认配置
2. **全局配置** — `~/.orangecoding/config.jsonc`
3. **项目配置** — `.opencode/orangecoding.jsonc`（当前目录）

---

## API Key 设置

OrangeCoding 支持多种 AI 提供者。你需要至少配置一个提供者的 API Key。
...
在 `.opencode/orangecoding.jsonc` 中直接设置：
...
OrangeCoding 会自动调用合适的工具（读取文件、搜索代码等）来完成你的请求。
...
首次使用建议执行深度初始化，让 OrangeCoding 全面了解你的项目：
...
启动 UltraWork 全自动模式，让 OrangeCoding 自主完成开发任务：
