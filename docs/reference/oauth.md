...
OrangeCoding 在 `orangecoding-cli` crate 的 `oauth.rs` 模块和 `orangecoding-mcp` crate 中实现了完整的 OAuth 2.1 流程，支持以下场景：
...
│                                                          │
│  ┌──────────┐     ┌──────────────┐     ┌──────────────┐ │
│  │ orangecoding-cli │────►│  OAuth 流程   │────►│  Token 存储  │ │
│  │ oauth.rs  │     │  PKCE + S256 │     │  加密存储     │ │
│  └──────────┘     └──────────────┘     └──────────────┘ │
│       │                                       │          │
│       ▼                                       ▼          │
│  ┌──────────┐                          ┌──────────────┐ │
│  │ orangecoding-mcp│                          │ orangecoding-config │ │
│  │ 服务器认证│                          │ CryptoStore  │ │
│  └──────────┘                          └──────────────┘ │
└──────────────────────────────────────────────────────────┘
```
...
Token 通过 `orangecoding-config` crate 的 `CryptoStore` 加密存储：
...
client_id = "orangecoding_abc123"  # 可选，不提供则自动注册
...
- [架构概览](../architecture/overview.md) - orangecoding-mcp 在系统中的位置
