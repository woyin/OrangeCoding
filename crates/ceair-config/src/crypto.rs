...
impl CryptoStore {
    ...
    pub fn default_store() -> ceair_core::Result<Self> {
    ...
    pub fn encrypt_value(&self, plaintext: &str, passphrase: &str) -> ceair_core::Result<Vec<u8>> {
        debug!("开始加密数据");
    ...
    pub fn decrypt_value(&self, encrypted_bytes: &[u8], passphrase: &str) -> ceair_core::Result<String> {
        debug!("开始解密数据");
    ...
    /// 从文件中检索并解密密钥
    pub fn retrieve_secret(&self, name: &str, passphrase: &str) -> ceair_core::Result<String> {
        debug!("检索密钥: {}", name);
    ...
    /// 加密值并返回加密数据结构（内部使用）
    fn encrypt_to_data(&self, plaintext: &str, passphrase: &str) -> ceair_core::Result<EncryptedData> {
        // 生成随机盐值
        let mut salt = [0u8; SALT_LEN];
        self.rng.fill(&mut salt).map_err(|_| {
            CeairError::internal("生成随机盐值失败")
    ...
    /// 从文件加载密钥存储
    fn load_store(&self) -> ceair_core::Result<SecretStore> {
    ...
    /// 将密钥存储保存到文件
    fn save_store(&self, store: &SecretStore) -> ceair_core::Result<()> {
        // 确保父目录存在
