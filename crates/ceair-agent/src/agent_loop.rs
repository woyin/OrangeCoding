...
use ceair_ai::provider::{
    AiProvider, AiResponse, ChatMessage, ChatOptions, MessageRole, ToolDefinition,
};
use ceair_ai::TokenUsage as AiTokenUsage;
use ceair_core::event::AgentEvent;
use ceair_core::message::{Message, ToolCall as CoreToolCall};
use ceair_core::types::{AgentId, ToolName};
...
impl AgentLoop {
    ...
    /// 失败时返回 `ceair_core::CeairError`。
    pub async fn run(
        &mut self,
        chat_options: &ChatOptions,
        cancel_token: CancellationToken,
        event_sender: mpsc::Sender<AgentEvent>,
    ) -> ceair_core::Result<AgentLoopResult> {
    ...
    async fn call_ai_provider(
        ...
        tokio::select! {
            ...
            result = self.provider.chat_completion(messages, tools, options) => {
                result.map_err(|e| ceair_core::CeairError::Ai {
            ...
            _ = cancel_token.cancelled() => {
                Err(ceair_core::CeairError::Agent {
    ...
    /// 将对话中的 ceair-core 消息转换为 AI 提供者的 ChatMessage 格式
    fn convert_to_chat_messages(&self) -> Vec<ChatMessage> {
        self.context
            ...
            .map(|msg| {
                match msg.role {
                    ceair_core::message::Role::System => {
                        ChatMessage::system(msg.content.clone().unwrap_or_default())
                    }
                    ceair_core::message::Role::User => {
                        ChatMessage::user(msg.content.clone().unwrap_or_default())
                    }
                    ceair_core::message::Role::Assistant => {
                        if msg.tool_calls.is_empty() {
                            // 纯文本助手消息
                            ChatMessage::assistant(msg.content.clone().unwrap_or_default())
                        } else {
                            // 带工具调用的助手消息
                            let ai_tool_calls: Vec<ceair_ai::ToolCall> = msg
                                .tool_calls
                                .iter()
                                .map(|tc| ceair_ai::ToolCall {
                                    id: tc.id.clone(),
                                    call_type: "function".to_string(),
                                    function: ceair_ai::provider::FunctionCall {
                    ...
                    ceair_core::message::Role::Tool => ChatMessage::tool_result(
    ...
    /// 将 AI 返回的工具调用转换为 ceair-core 的 ToolCall 格式
    fn convert_ai_tool_calls(&self, ai_calls: &[ceair_ai::ToolCall]) -> Vec<CoreToolCall> {
        ai_calls
            .iter()
            .map(|ac| {
        ...
        schemas_array
            ...
            .filter_map(|schema| {
                ...
                Some(ToolDefinition {
                    ...
                    function: ceair_ai::provider::FunctionDefinition {
                        name,
                        description,
                        parameters: ceair_ai::provider::ToolParameter {
...
mod tests {
    ...
    impl AiProvider for MockTextProvider {
        ...
        async fn chat_completion_stream(
            ...
            _tools: &[ToolDefinition],
            _options: &ChatOptions,
        ) -> AiResult<ceair_ai::provider::StreamResponse> {
    ...
    impl AiProvider for MockToolCallProvider {
        ...
        async fn chat_completion(
            ...
            if count == 0 {
                ...
                Ok(AiResponse {
                    content: String::new(),
                    tool_calls: vec![ceair_ai::ToolCall {
    ...
    #[async_trait::async_trait]
    impl ceair_tools::Tool for TestTool {
        fn name(&self) -> &str {
            "test_tool"
        }
